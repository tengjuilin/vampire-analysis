...
@pytest.mark.parametrize('A, expected_V, expected_T, expected_d', [
    ...
    # 2 x 2 zero
    (np.array([[0, 0], [0, 0]]), np.array([[1, 0], [0, 1]]), np.array([[0, 0], [0, 0]]), np.array([0, 0])),
    # 2 x 2 identity
    (np.array([[1, 0], [0, 1]]),
    ...
    # 2 x 3
    (np.array([[7.2, 2.1, 1], [12, 2.1, 30]]),
     np.array([[0.16329553624, 0], [0, 1], [0.986577198117, 0]]),
     np.array([[-14.69727865967, 0], [14.69727865967, 0]]),
     np.array([432.02, 0])),
])
...
@pytest.mark.parametrize('A, expected_V, expected_T, expected_d', [
    # test for m x n matrix where m >= n
    # 2 x 2 zero
    (np.array([[0, 0], [0, 0]]), np.array([[0, 1], [1, 0]]), np.array([[0, 0], [0, 0]]), np.array([0, 0])),
    # 2 x 2 identity
    (np.array([[1, 0], [0, 1]]),
     np.array([[-0.707106781187, -0.707106781187], [0.707106781187, -0.707106781187]]),
     np.array([[-0.707106781187, 0], [0.707106781187, 0]]),
    ...
    (np.array([[3, -2], [5, 1.2]]),
     ...
     np.array([7.12000000e+00, 0])),
    # 3 x 2
    (np.array([[2.3, -5.1], [32, -3.1], [6.9, 90]]),
